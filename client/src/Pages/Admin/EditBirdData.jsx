...
import { FaTimes, FaSave, FaArrowLeft, FaPlus } from "react-icons/fa";
...
const EditBirdData = () => {
    ...
    const fetchBirdData = async () => {
      try {
        ...
        setFormData({
          ...
          habitatMap: birdData.habitatMap || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3001/api/birds/edit/${id}`, submissionData);
      
      if (response.data.message === "Bird updated successfully") {
        navigate("/admin/bird-data", { state: { message: "Bird updated successfully!" } });
      } else {
        setError(response.data.message || "Failed to update bird");
      }
    } catch (err) {
      ...
      if (err.response) {
        ...
        setError("Failed to update bird. Please try again.");
      }
    } finally {
      setSaving(false);
    }
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600">
                  Update information for {formData.primaryName}
                </p>
              </div>
            </div>
          </div>
